...
class TestSensitivePathCheck:
    """Protect system paths without blocking user-owned Hermes config."""

    def test_hermes_config_allowed_for_write_file(self, tmp_path):
        fake_config = tmp_path / "config.yaml"

        from tools.file_tools import write_file_tool
        content = "model:\n  default: gpt-test\n"
        result = json.loads(write_file_tool(str(fake_config), content))
        assert not result.get("error"), result
        assert fake_config.read_text() == content

    def test_hermes_config_allowed_for_patch(self, tmp_path):
        fake_config = tmp_path / "config.yaml"
        fake_config.write_text("model:\n  default: gpt-old\n")

        from tools.file_tools import patch_tool
        result = json.loads(patch_tool(
            mode="replace",
            path=str(fake_config),
            old_string="default: gpt-old",
            new_string="default: gpt-new",
        ))
        assert not result.get("error"), result
        assert "default: gpt-new" in fake_config.read_text()

    def test_system_path_still_blocked(self):
        from tools.file_tools import write_file_tool
    ...
    @patch("tools.file_tools._get_file_ops")
    def test_normal_file_not_blocked(self, mock_get):
        mock_ops = MagicMock()
        result_obj = MagicMock()
