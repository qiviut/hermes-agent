...
危险命令审批会一直保持待处理，直到用户明确批准、拒绝，或主动中断/停止会话。这同样适用于 CLI 和 Gateway 命令审批、ACP 权限请求、MCP 征询同意，以及 Matrix、Discord、QQ 等消息平台。沉默既不表示同意，也不会被自动转换为拒绝。中断会作为独立的故障关闭结果返回，而不会被改写成用户拒绝；相应的 ACP future 与 MCP 同步同意等待器也会被取消或释放。中断在与同时到达的批准发生竞争时优先。只有精确的 `once`、`session` 和 `always` 值可以授予权限；格式错误、未知或拼写错误的回调/传输值都会故障关闭。

Gateway 会在等待期间发送活动心跳，避免健康的后台任务被误判为空闲并终止。MCP 传输超时不会计算等待征询同意的时间；决定后普通网络超时会继续生效。由于 MCP 征询在会话接收循环而不是工具调用任务上运行，原始调用被中断时 Hermes 还会使用每次调用独立的取消令牌显式通知该等待器；排队的并发调用不能清除或消费其他调用的同意信号。若平台按钮因交互令牌限制而过期，审批本身仍然待处理，可使用文本命令 `!approve` / `!deny` 或 `/approve` / `/deny` 继续处理。显式停止会话或进程重启仍会中断运行，客户端应清楚展示该状态并使用正常的恢复路径。

Runs 审批端点还接受 `safer_alternative`。这不是批准：原请求机制仍会被阻止，不会创建任何可复用权限；工具结果只允许在原任务已有授权范围内改用更安全的原生、专用或托管方法。
